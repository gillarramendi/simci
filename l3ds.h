...
public:
  ...
  float GetShininess();
  // returns the transparency of the material, ranging from 1(fully transparent)
  // to 0(opaque)
  ...
  // this methods should not be used by the "user", they're used internally to
  // fill the class with valid data when reading from file. If you're about to
  // add an importer for another format you'LL have to use these methods sets
  // the material ID to "value"
...
public:
  ...
  // sets the texture coordinates vector at a given index to "vec" - for
  // internal use
  ...
  // returns the triangle with a given index, see LTriangle2 structure
  // description
  ...
  // sets an internal triangle structure with index "index" - for internal use
  // only
  void SetTri(const LTri &tri, uint index);
  // returns the pointer to the internal triangle structure - for internal use
  // only
...
protected:
  ...
  // calculates the normals, either using the smoothing groups information or
  // not
...
public:
  ...
  // returns the pointer to the material with a given name, or NULL if the
  // material was not found
  LMaterial *FindMaterial(const std::string &name);
  // returns the pointer to the mesh with a given name, or NULL if the mesh with
  // such name is not present in the scene
...
protected:
  ...
  // reads the face list, face materials, smoothing groups... and fill rthe
  // information into the mesh
  ...
  // reads the keyheader structure from the current offset and returns the frame
  // number
  long ReadKeyheader();
};
