...
void LObject::SetName(const std::string &value) { m_name = value; }

const std::string &LObject::GetName() { return m_name; }

bool LObject::IsObject(const std::string &name) { return (m_name == name); }

//-------------------------------------------------------
// LMaterial implementation
//-------------------------------------------------------

LMaterial::LMaterial() : LObject() {
  m_id = 0;
  m_texMap1 = emptyMap;
  m_texMap2 = emptyMap;
  m_opacMap = emptyMap;
...
uint LMaterial::GetID() { return m_id; }

LMap &LMaterial::GetTextureMap1() { return m_texMap1; }

LMap &LMaterial::GetTextureMap2() { return m_texMap2; }

LMap &LMaterial::GetOpacityMap() { return m_opacMap; }

LMap &LMaterial::GetSpecularMap() { return m_specMap; }

LMap &LMaterial::GetBumpMap() { return m_bumpMap; }

LMap &LMaterial::GetReflectionMap() { return m_reflMap; }

LColor3 LMaterial::GetAmbientColor() { return m_ambient; }

LColor3 LMaterial::GetDiffuseColor() { return m_diffuse; }

LColor3 LMaterial::GetSpecularColor() { return m_specular; }

float LMaterial::GetShininess() { return m_shininess; }

float LMaterial::GetTransparency() { return m_transparency; }

LShading LMaterial::GetShadingType() { return m_shading; }

void LMaterial::SetID(uint value) { m_id = value; }

void LMaterial::SetAmbientColor(const LColor3 &color) { m_ambient = color; }

void LMaterial::SetDiffuseColor(const LColor3 &color) { m_diffuse = color; }

void LMaterial::SetSpecularColor(const LColor3 &color) { m_specular = color; }
...
void LMaterial::SetShadingType(LShading shading) { m_shading = shading; }

//-------------------------------------------------------
// LMesh implementation
//-------------------------------------------------------

LMesh::LMesh() : LObject() { Clear(); }
...
uint LMesh::GetVertexCount() { return m_vertices.size(); }
...
uint LMesh::GetTriangleCount() { return m_triangles.size(); }
...
const LVector4 &LMesh::GetVertex(uint index) { return m_vertices[index]; }

const LVector3 &LMesh::GetNormal(uint index) { return m_normals[index]; }

const LVector2 &LMesh::GetUV(uint index) { return m_uv[index]; }

const LVector3 &LMesh::GetTangent(uint index) { return m_tangents[index]; }

const LVector3 &LMesh::GetBinormal(uint index) { return m_binormals[index]; }
...
const LTriangle &LMesh::GetTriangle(uint index) { return m_triangles[index]; }
...
LMatrix4 LMesh::GetMatrix() { return m_matrix; }

void LMesh::SetMatrix(LMatrix4 m) { m_matrix = m; }
...
void LMesh::CalcNormals(bool useSmoothingGroups) {
  ...
  for (i = 0; i < m_triangles.size(); i++) {
    LVector3 a, b;
    a = SubtractVectors(_4to3(m_vertices[m_tris[i].b]),
                        _4to3(m_vertices[m_tris[i].a]));
    b = SubtractVectors(_4to3(m_vertices[m_tris[i].b]),
                        _4to3(m_vertices[m_tris[i].c]));
  ...
  if (!useSmoothingGroups) {
    ...
    for (i = 0; i < loop_size; i++) {
      ...
      // now duplicate the vertices so that there's only one smoothing group
      // "per vertex"
      if (smGroups.size() > 1)
...
void LMesh::CalcTextureSpace() {
  ...
  std::vector<std::vector<int>> array;
  array.resize(m_vertices.size());
  for (i = 0; (unsigned int)i < m_triangles.size(); i++) // GOTZON
  {
    uint k = m_tris[i].a;
    array[k].push_back(i);
  ...
  for (i = 0; (unsigned int)i < m_vertices.size(); i++) // GOTZON
  {
    v1 = zero3;
    v2 = zero3;
    int t = array[i].size();
...
LTri &LMesh::GetTri(uint index) { return m_tris[index]; }

uint LMesh::GetMaterial(uint index) { return m_materials[index]; }
...
uint LMesh::GetMaterialCount() { return m_materials.size(); }

//-------------------------------------------------------
// LCamera implementation
//-------------------------------------------------------

LCamera::LCamera() : LObject() { Clear(); }
...
void LCamera::SetPosition(LVector3 vec) { m_pos = vec; }

LVector3 LCamera::GetPosition() { return m_pos; }

void LCamera::SetTarget(LVector3 target) { m_target = target; }

LVector3 LCamera::GetTarget() { return m_target; }

void LCamera::SetFOV(float value) { m_fov = value; }

float LCamera::GetFOV() { return m_fov; }

void LCamera::SetBank(float value) { m_bank = value; }

float LCamera::GetBank() { return m_bank; }

void LCamera::SetNearplane(float value) { m_near = value; }

float LCamera::GetNearplane() { return m_near; }

void LCamera::SetFarplane(float value) { m_far = value; }

float LCamera::GetFarplane() { return m_far; }

//-------------------------------------------------------
// LLight implementation
//-------------------------------------------------------

LLight::LLight() : LObject() { Clear(); }
...
void LLight::SetPosition(LVector3 vec) { m_pos = vec; }

LVector3 LLight::GetPosition() { return m_pos; }

void LLight::SetColor(LColor3 color) { m_color = color; }

LColor3 LLight::GetColor() { return m_color; }

void LLight::SetSpotlight(bool value) { m_spotlight = value; }

bool LLight::GetSpotlight() { return m_spotlight; }

void LLight::SetTarget(LVector3 target) { m_target = target; }

LVector3 LLight::GetTarget() { return m_target; }

void LLight::SetHotspot(float value) { m_hotspot = value; }

float LLight::GetHotspot() { return m_hotspot; }

void LLight::SetFalloff(float value) { m_falloff = value; }

float LLight::GetFalloff() { return m_falloff; }

void LLight::SetAttenuationstart(float value) { m_attenuationend = value; }

float LLight::GetAttenuationstart() { return m_attenuationend; }

void LLight::SetAttenuationend(float value) { m_attenuationstart = value; }

float LLight::GetAttenuationend() { return m_attenuationstart; }

//-------------------------------------------------------
// LImporter implementation
//-------------------------------------------------------

LImporter::LImporter() { Clear(); }

LImporter::~LImporter() { Clear(); }

uint LImporter::GetMeshCount() { return m_meshes.size(); }

uint LImporter::GetLightCount() { return m_lights.size(); }

uint LImporter::GetMaterialCount() { return m_materials.size(); }

uint LImporter::GetCameraCount() { return m_cameras.size(); }

LMesh &LImporter::GetMesh(uint index) { return m_meshes[index]; }

LLight &LImporter::GetLight(uint index) { return m_lights[index]; }

LMaterial &LImporter::GetMaterial(uint index) { return m_materials[index]; }

LCamera &LImporter::GetCamera(uint index) { return m_cameras[index]; }
...
LOptimizationLevel LImporter::GetOptimizationLevel() { return m_optLevel; }
...
L3DS::L3DS(const char *filename) : LImporter() {
  m_buffer = 0;
  m_bufferSize = 0;
  m_pos = 0;
  m_eof = false;
...
void L3DS::SkipChunk(const LChunk &chunk) { Seek(chunk.end, SEEK_START); }

void L3DS::GotoChunk(const LChunk &chunk) { Seek(chunk.start, SEEK_START); }
